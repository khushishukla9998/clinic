...
const getAvailableSlots = async (req, res) => {
    try {
        ...
        if (!date) {
            return commonUtils.sendErrorResponse(req, res, "Date is required (format: YYYY-MM-DD).", null, 400);
        }

        if (!moment(date, "YYYY-MM-DD", true).isValid()) {
            return commonUtils.sendErrorResponse(req, res, "Invalid date format. Use YYYY-MM-DD.", null, 400);
        }

    
        if (moment(date, "YYYY-MM-DD").isBefore(moment().startOf("day"))) {
            return commonUtils.sendErrorResponse(req, res, "Cannot check slots for past dates.", null, 400);
        }
        ...
        if (!doctor) {
            return commonUtils.sendErrorResponse(req, res, "Doctor not found.", null, 404);
        }
        ...
        if (!step3 || !step3.data || !step3.data.availableTimeSlots) {
            return commonUtils.sendErrorResponse(req, res, "Doctor has not set available time slots.", null, 400);
        }
        ...
        if (daySlots.length === 0) {
            return commonUtils.sendSuccessResponse(req, res, "Doctor is not available on this day.", { availableSlots: [] });
        }
        ...
        for (const leave of leaves) {
            if (leave.unavailableDates && leave.unavailableDates instanceof Map) {
                ...
                if (dateLeave) {
                    if (dateLeave.category === "full_day") {
        ...
        if (isOnFullDayLeave) {
            return commonUtils.sendSuccessResponse(req, res, "Doctor is on leave on this date.", { availableSlots: [] });
        }
...
const bookAppointment = async (req, res) => {
    try {
        ...
        if (moment(date, "YYYY-MM-DD").isBefore(moment().startOf("day"))) {
            return commonUtils.sendErrorResponse(req, res, "Cannot book appointments for past dates.", null, 400);
        }
        ...
        if (!doctor) {
            return commonUtils.sendErrorResponse(req, res, "Doctor not found or not verified.", null, 404);
        }
        ...
        for (const leave of leaves) {
            if (leave.unavailableDates && leave.unavailableDates instanceof Map) {
                ...
                if (dateLeave) {
                    if (dateLeave.category === "full_day") {
                        return commonUtils.sendErrorResponse(req, res, "Doctor is on leave on this date.", null, 400);
                    }
                    if (dateLeave.slots && dateLeave.slots.length > 0) {
                        ...
                        if (isOnLeave) {
                            return commonUtils.sendErrorResponse(req, res, "Doctor is on leave during this time slot.", null, 400);
                        }
                    }
                }
            }
        ...
        return commonUtils.sendSuccessResponse(req, res, "Appointment booked successfully.", appointment, 201);
    } catch (error) {
        return commonUtils.sendErrorResponse(req, res, error.message, null, 500);
    }
};
